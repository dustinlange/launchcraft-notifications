...
export function upsertSubscription(db: D1Database, sub: Pick<Subscription, 'launch_id' | 'device_token' | 'user_id'>) {
  return db.prepare(`
    INSERT INTO subscriptions (launch_id, device_token, user_id)
    VALUES (?, ?, ?)
    ON CONFLICT(launch_id, user_id) DO UPDATE SET device_token = excluded.device_token
  `).bind(sub.launch_id, sub.device_token, sub.user_id).run()
}
