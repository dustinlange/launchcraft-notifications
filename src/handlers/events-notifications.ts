...
export async function dispatchEventNotifications(env: Env): Promise<void> {
  ...
  // Upsert all events so we have fresh dates
  ...
  // Check each upcoming event for 24h and 1h notification windows
  for (const e of ll2Events) {
    ...
    const windows: Array<{ label: '24h' | '1h'; fromS: number; toS: number; text: string }> = [
    ...
    for (const window of windows) {
      ...
      const { results: users } = await getUsersForEventType(env.DB, e.type?.id ?? null)
      if (users.length === 0) {
